...
def prompt_for_device(iot, subcommand):
    ...
    click.secho("\nProcessing IoTHub Edge Device")
    existingDevice = None
    while existingDevice is None:
        if not iot.config['device']:
            name = click.prompt("Enter an IoTHub Edge Device name")
        else:
            name = iot.config['device']

        click.secho("Checking for IoTHub Edge Device with name '%s'" % name)
        ...
        for d in existingDevices:
            if d["deviceId"] == name:
                existingDevice = d
                click.secho("Using existing IoTHub Edge Device with name '%s'" % name)

        if existingDevice is None:
            click.secho("IoTHub Edge Device with name '%s' does not exist. Creating a new IoTHub Edge Device..." % name)
...
def cli(ctx, wifi_ssid, wifi_password, resource_group, iothub, iothub_sku, device, container_registry, container_registry_sku,
    ...
    if ctx.obj.config['ip'] == DEFAULT_WIFI_AP_ADDRESS:
        # if the IP is default (192.168.4.1), then we assume it's WiFi
        if ctx.obj.config['wifi_ssid'] == "":
